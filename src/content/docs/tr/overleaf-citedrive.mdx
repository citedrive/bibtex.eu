---
title: "Overleaf + CiteDrive - LaTeX Kullanıcıları için Güçlü Bir Birleşim"
description: "CiteDrive, Overleaf için bulut odaklı bir referans ve alıntı yönetim platformudur."
template: splash
hero:
  title: "LaTeX Kullanıcıları için Güçlü Bir Birleşim"
  tagline: "Overleaf ve CiteDrive'ın güçlü bir birleşimi ile araştırma sürecinizi tamamlayın. Projeyi her iki uygulama arasında bağlayın ve Overleaf makaleniz, BibTeX dosyanızdan sadece birkaç tıklama ile otomatik olarak güncellenir."
  image:
    ...
    alt: "CiteDrive + Overleaf: Referans Yönetimi"
  actions:
    - text: ÜCRETSİZ DENEYİN
...
<Card title="Overleaf için Mükemmel!" >
  Overleaf, öğrenci ve araştırmacı takımlarının herhangi bir yerden aynı anda makale yazmalarını sağlayan kolay kullanımlı bir çevrimiçi LaTeX düzenleyicisidir. Overleaf, referanslarınızı yönetebileceğiniz BibTeX dahil çeşitli paketleri destekler. Overleaf'i CiteDrive'a bağladığınızda, projenizi ve BibTeX veritabanınızı Overleaf ile otomatik olarak senkronize edebilirsiniz.
</Card>

<CardGrid stagger>
  <Card title="Doğal BibTeX Desteği" icon="approve-check">
    Altyapımızı uyumlu BibTeX teslimatını sağlamak için tasarladık. Tüm resmi giriş ve alan tiplerini destekliyoruz; bunlar, tarayıcı eklentimizi kullanarak bir tıklamayla kolayca girilebilir veya ithal edilebilir: sıkıcı dönüştürücüler veya manuel işlemler gerekmez. Projeleriniz her zaman Overleaf için güncel tutulur ve LaTeX'in destekleyeceği bir şekilde sunulur.
  </Card>
  <Card title="Toplama ve Düzenleme" icon="approve-check">
    Tek bir projede çalışan büyük ekipler, BibTeX veritabanlarını (.bib dosyası) manuel olarak güncelleme konusunda hızlı bir şekilde karmaşaya ve hayal kırıklığına uğrayabilir, ve hatalar hızlı bir şekilde ortaya çıkabilir. CiteDrive ile tarayıcı uzantısı sayesinde bir tıklamayla referans ekleyebilir ve sürükleyip bırakarak referansları düzenli bir şekilde gruplandırabilirsiniz.
  </Card>
  <Card title="Tamamen İşbirliğine Uygun" icon="approve-check">
    Bilim adamları için takım çalışmasının ne kadar önemli olduğunu biliyoruz. Takımlar için kapsamlı araçlar artık yaygın hale geldi ve en önemlisi, makaleler nadiren yalnız yazılır. Bu nedenle platformumuzu işbirliği için inşa ettik. Projeye istediğiniz zaman meslektaşlarınızı ekleyin. Siz ve ekibiniz çalışmanız için referansları ve materyalleri toplar, yapılandırır ve açıklamalar ekler, hepsi net bir şekilde bir projede düzenlenmiş olarak.
  </Card>
